...
contract('ChainList', function(accounts) {
  ...
  it("should let us sell a second article", function() {
    return ChainList.deployed().then(function(instance) {
      ...
      assert.equal(data[0].toNumber(), 1, "article id for the first article must still be 1")
      return chainListInstance.articles(articleId);
    }).then(function(data) {
      ...
      assert.equal(web3.toBigNumber(data[5]), web3.toWei(articlePrice2, "ether"), "article price must be " + web3.toWei(articlePrice2, "ether"));
    });
  });
  ...
  it("should let us buy the first article", function() {
    return ChainList.deployed().then(function(instance) {
      ...
      assert(data[0], 2, "article 2 should be the only article left for sale")
    });
  });
});
